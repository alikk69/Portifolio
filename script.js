...
// Same idea as careerScreenPoint, but for .career-chain-svg, which lives
// INSIDE .career-canvas now (see style.css .career-chain-svg for why) —
// so its own coordinate origin is .career-canvas's own box, not
// .career-explorer's. .career-nav's offsetLeft/offsetTop are already
// relative to .career-canvas (its nearest positioned ancestor, since
// .career-canvas-scroll in between is position:static), which is exactly
// the frame this svg is drawn in — so this is careerScreenPoint with the
// .career-canvas offset term dropped, not re-derived from scratch.
function careerCanvasPoint(worldX, worldY) {
  return {
    x: careerNavEl.offsetLeft + worldX * careerZoom - careerCanvasScrollEl.scrollLeft,
    y: careerNavEl.offsetTop + worldY * careerZoom - careerCanvasScrollEl.scrollTop,
  };
}

function careerChainNodeCenter(node) {
  return careerCanvasPoint(node.offsetLeft + node.offsetWidth / 2, node.offsetTop + node.offsetHeight / 2);
}
...
function updateCareerChain() {
  const svg = document.querySelector(".career-chain-svg");
  if (!svg || !careerCanvasEl || !careerNavEl) return;

  for (let i = 0; i < careerNavItems.length - 1; i++) {
    const p1 = careerChainNodeCenter(careerNavItems[i]);
    const p2 = careerChainNodeCenter(careerNavItems[i + 1]);
...
function showCareerPanel(targetId) {
  ...
  updateCareerEdge();
  updateCareerToolbar();
  syncCareerPanelHeight();
}

// Mobile only: on desktop each role swaps via display:none, so the panel
// naturally sizes to whichever ONE item is showing. On mobile all six sit
// side by side in one scroll-snap flex row instead — and a flex row's own
// height defaults to its TALLEST child regardless of align-items, so
// every card sat inside a track sized for whichever role has the most to
// say (Nadher Media, with two stat chips and three paragraphs), leaving
// as much as ~300px of blank card face below shorter roles' text (Poly's
// two-sentence blurb, especially — measured, not estimated). This locks
// the track's visible height to whichever card is actually showing and
// animates smoothly between them (see .career-panel's mobile
// `transition: height`) — the align-items: flex-start override in that
// same media query is what makes each item's own scrollHeight reflect
// its OWN content instead of the stretched-to-match-the-tallest value.
function syncCareerPanelHeight() {
  if (!careerPanelEl) return;
  // Crossing back above the breakpoint (resize, rotation) has to actively
  // clear the inline height, not just stop touching it — desktop's own
  // auto-sizing (one item shown via display swap) would otherwise stay
  // stuck at whatever the last mobile card's pixel height happened to be.
  if (window.innerWidth > 820) {
    careerPanelEl.style.height = "";
    return;
  }
  const active = document.querySelector(".career-panel-item.is-active") || careerPanelItems[0];
  if (active) careerPanelEl.style.height = `${active.scrollHeight}px`;
}
...
// The role-chip row's own "swipe for more" cue (see the HTML comment by
// #careerChipSwipeCue) — independent of updateCareerSwipeCue() below,
// which tracks .career-panel (the card carousel), a SEPARATE scroll
// container that doesn't move in sync with the chip row on a raw swipe
// (only tapping a chip syncs the two). Same "hide once nothing's left to
// scroll to" logic as updateCareerNavScroll() above, since both watch
// the same element.
const careerChipSwipeCue = document.getElementById("careerChipSwipeCue");
if (careerChipSwipeCue && careerNavEl) {
  function updateCareerChipSwipeCue() {
    const scrollable = careerNavEl.scrollWidth - careerNavEl.clientWidth;
    const atEnd = scrollable <= 4 || careerNavEl.scrollLeft >= scrollable - 4;
    careerChipSwipeCue.classList.toggle("is-hidden", atEnd);
  }
  careerNavEl.addEventListener("scroll", updateCareerChipSwipeCue, { passive: true });
  window.addEventListener("resize", updateCareerChipSwipeCue);
  updateCareerChipSwipeCue();
}
...
const kpiSwipeCue = document.getElementById("kpiSwipeCue");
if (tickerBoard && tickerDots.length) {
  // Same "stays put until the last stat is actually reached" logic as
  // updateCareerSwipeCue() — a cue that vanishes on first touch doesn't
  // help someone who swiped once by accident and wants to know if
  // there's more.
  function updateKpiSwipeCue() {
    if (!kpiSwipeCue) return;
    const atEnd = tickerBoard.scrollLeft >= tickerBoard.scrollWidth - tickerBoard.clientWidth - 4;
    kpiSwipeCue.classList.toggle("is-hidden", atEnd);
  }
  ...
  tickerBoard.addEventListener(
    ...
    () => {
      ...
      tickerScrollRaf = requestAnimationFrame(() => {
        updateActiveTickerDot();
        updateKpiSwipeCue();
      });
    },
  ...
  window.addEventListener("resize", updateKpiSwipeCue);
  updateKpiSwipeCue();
...
if (creditCard && ccScrollEl) {
  ...
  // settle with a slight spring overshoot — a standard easeOutBack, tuned
  // subtle (c1 1.1, not the usual 1.70158) so it reads as "arrived with a
  // touch of physicality" rather than a cartoon bounce. Still a PURE
  // function of scroll position p, same as the whole reveal always has
  // been — no timers, no animation state — the overshoot just means mech
  // briefly exceeds 1 partway through the last quarter of scroll before
  // landing exactly on 1 at p=1, same as before. That "exactly 1 at rest"
  // is load-bearing: CARD_TUCK_Y/CARD_REVEAL_Y and the wallet clearance
  // were measured against mech===1 being the true final state, and this
  // curve still guarantees that boundary — only the path there changed.
  function cardMech(p) {
    if (p < 0.75) return (p / 0.75) * 0.82;
    const t = (p - 0.75) / 0.25;
    // c1=3 measured to a ~4.5% peak overshoot on mech, landing around
    // p=0.876 — translates to roughly 8px of extra lift at the peak
    // (mech multiplies the 178px CARD_TUCK_Y→CARD_REVEAL_Y range) before
    // settling. That direction is the safe one: overshoot only ever
    // pushes the card FURTHER from the wallet (more negative Y), never
    // back toward it, so it can't reintroduce the pocket-clearance bugs
    // this component has a real history of — see CARD_TUCK_Y above.
    const c1 = 3;
    const c3 = c1 + 1;
    const eased = 1 + c3 * Math.pow(t - 1, 3) + c1 * Math.pow(t - 1, 2);
    return 0.82 + eased * 0.18;
  }
...
if (gsapReady && !prefersReducedMotion) {
  ScrollTrigger.matchMedia({
    "(min-width: 821px) and (min-height: 700px)": function () {
      ...
      // Was "+=200%", then "+=100%" (owner feedback both times: still too
      // much scrolling to get through). GSAP resolves a bare "%" end
      // offset against the VIEWPORT height, not the section's own height,
      // so 200% was 2 full screens of pinned, non-scrolling scroll
      // distance spent on nothing but 3 already-visible cards swapping
      // which one is emphasized. 60% keeps enough range for scrub's
      // smoothing to still read as a deliberate scroll-driven reveal
      // rather than a jump cut (verify with ScrollTrigger.getAll().find(t
      // => t.trigger.id === 'achievements') at a real viewport size before
      // pushing this any lower — scrub needs *some* physical distance to
      // smooth over, or the catch-up lag starts to look like stutter
      // instead of silkiness).
...
if (gsapReady && !prefersReducedMotion) {
  function scrollReveal(targets, vars = {}) {
    ...
    // scale + filter layer the same "settling into focus" arrival used by
    // .reveal (CSS, style.css) on top of GSAP's finer per-element stagger —
    // one consistent motion language whether an element's entrance is
    // driven by the IntersectionObserver or by GSAP.
    gsap.set(els, { opacity: 0, y: 50, scale: 0.94, filter: "blur(6px)", ...vars.from });
    els.forEach((el, i) => {
      gsap.to(el, {
        opacity: 1,
        y: 0,
        ...
        ease: "power3.out",
        ...vars.to,
        // GSAP leaves the tweened value (blur(0px)) sitting as an inline
        // style once the tween finishes — unlike .reveal.in-view (CSS),
        // which can just declare the literal keyword "none". blur(0px) is
        // NOT the same thing as none: per spec, any filter value other
        // than the exact keyword "none" still creates a containing block
        // for fixed/sticky descendants, the identical hazard the .reveal
        // comment above documents for transform. None of today's
        // scrollReveal() targets contain a fixed/sticky descendant, so
        // this isn't live yet — but it would be the instant one did,
        // silently, unless the inline style is actually removed (not just
        // zeroed) once it's done its job.
...
// ---- Mobile nav: a card dropping from the header over a dimmed backdrop,
// for the collapsed <860px menu ----
// #mobileNav (see index.html / style.css) is a separate element from the
// desktop #primaryNav, not that same nav reshaped — #navToggle's existing
// hamburger/X button is reused as-is to drive it, unchanged.
const navToggle = document.getElementById("navToggle");
const mobileNav = document.getElementById("mobileNav");
const mobileNavPanel = mobileNav && mobileNav.querySelector(".mobile-nav-panel");
if (navToggle && mobileNav) {
  ...
  function openNav() {
    ...
    // Also hides .back-to-top (see its rule in style.css) — a "scroll to
    // top" button floating above the dimmed backdrop would look broken,
    // and there's nothing to scroll while this is open anyway.
    document.documentElement.classList.add("nav-is-open");
    // The backdrop covers the whole screen even though the card doesn't,
    // so the page behind it has no business scrolling underneath — same
    // lock the achievement modal and the intro overlay already use
  ...
  // Tapping the dimmed backdrop closes the menu, same as any iOS/Android
  // sheet — #mobileNav is the full-viewport hit area (background scrim +
  // padding), .mobile-nav-panel is the actual card, so "landed outside
  // the card" is just "the click target isn't inside the panel."
  mobileNav.addEventListener("click", (e) => {
    if (mobileNavPanel && !mobileNavPanel.contains(e.target)) closeNav();
  });
  ...
  // Crossing back above the breakpoint (resize, rotation) with the menu
  // open would otherwise leave body scroll locked forever, since nothing
  // else would ever call closeNav() to release it — .mobile-nav itself
  // disappears via display:none in the media query regardless, but the
  // scroll lock is a separate, JS-owned piece of state that query can't
  // reach.
  window.addEventListener("resize", () => {
    if (window.innerWidth > 860 && mobileNav.classList.contains("is-open")) closeNav();
  });
}
